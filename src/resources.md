...
| uuid | Unieke resource identifier (UUID4) | string | ja | ~~C~~​R​~~U~~​~~D~~ |
...
* `medewerker` - Medewerker |  | ja | C​R​U​D |
...
| omschrijving | Omschrijving van de aard van de ROL, afgeleid uit het ROLTYPE. | string | ja | ~~C~~​R​~~U~~​~~D~~ |
...
| registratiedatum | De datum waarop dit object is geregistreerd. | string | ja | ~~C~~​R​~~U~~​~~D~~ |
...
* `machtiginggever` - De betrokkene in de rol bij de zaak heeft een andere betrokkene bij dezelfde zaak gemachtigd om namens hem of haar te handelen |  | nee | C​R​U​D |
| contactpersoonRol | De gegevens van de persoon die anderen desgevraagd in contact brengt met medewerkers van de BETROKKENE, een NIET-NATUURLIJK PERSOON of VESTIGING zijnde, of met BETROKKENE zelf, een NATUURLIJK PERSOON zijnde , vanuit het belang van BETROKKENE in haar ROL bij een ZAAK. |  | nee | C​R​U​D |
| statussen | De BETROKKENE die in zijn/haar ROL in een ZAAK heeft geregistreerd dat STATUSsen in die ZAAK bereikt zijn. | array | ja | ~~C~~​R​~~U~~​~~D~~ |
...
| einddatum | De datum waarop de uitvoering van de zaak afgerond is. | string | ja | ~~C~~​R​~~U~~​~~D~~ |
...
| vertrouwelijkheidaanduiding | Aanduiding van de mate waarin het zaakdossier van de ZAAK voor de openbaarheid bestemd is. Optioneel - indien geen waarde gekozen wordt, dan wordt de waarde van het ZAAKTYPE overgenomen. Dit betekent dat de API _altijd_ een waarde teruggeeft. |  | nee | C​R​U​D |
...
* `geheel` - De met de zaak gemoeide kosten zijn geheel betaald. |  | nee | C​R​U​D |
| betalingsindicatieWeergave | Uitleg bij `betalingsindicatie`. | string | ja | ~~C~~​R​~~U~~​~~D~~ |
...
| zaakgeometrie | Punt, lijn of (multi-)vlak geometrie-informatie, in GeoJSON. |  | nee | C​R​U​D |
| verlenging | Gegevens omtrent het verlengen van de doorlooptijd van de behandeling van de ZAAK |  | nee | C​R​U​D |
| opschorting | Gegevens omtrent het tijdelijk opschorten van de behandeling van de ZAAK |  | nee | C​R​U​D |
...
| deelzaken | URL-referenties naar deel ZAAKen. | array | ja | ~~C~~​R​~~U~~​~~D~~ |
| relevanteAndereZaken | Een lijst van relevante andere zaken. | array | nee | C​R​U​D |
| eigenschappen |  | array | ja | ~~C~~​R​~~U~~​~~D~~ |
| rollen |  | array | ja | ~~C~~​R​~~U~~​~~D~~ |
| status | Indien geen status bekend is, dan is de waarde &#x27;null&#x27; | string | ja | ~~C~~​R​~~U~~​~~D~~ |
| zaakinformatieobjecten |  | array | ja | ~~C~~​R​~~U~~​~~D~~ |
| zaakobjecten |  | array | ja | ~~C~~​R​~~U~~​~~D~~ |
...
* `overgedragen` - De zaak cq. het zaakdossier is overgebracht naar een archiefbewaarplaats. |  | nee | C​R​U​D |
...
| resultaat | URL-referentie naar het RESULTAAT. Indien geen resultaat bekend is, dan is de waarde &#x27;null&#x27; | string | ja | ~~C~~​R​~~U~~​~~D~~ |
...
| processobject | Specificatie van de attribuutsoort van het object, subject of gebeurtenis  waarop, vanuit archiveringsoptiek, de zaak betrekking heeft en dat bepalend is voor de start van de archiefactietermijn. |  | nee | C​R​U​D |
...
| naam | De naam van de EIGENSCHAP (overgenomen uit de Catalogi API). | string | ja | ~~C~~​R​~~U~~​~~D~~ |
...
| zaak | URL-referentie naar de ZAAK. | string | ja | C​R​U​D |
| aardRelatieWeergave |  |  | ja | ~~C~~​R​~~U~~​~~D~~ |
...
| registratiedatum | De datum waarop de behandelende organisatie het INFORMATIEOBJECT heeft geregistreerd bij het OBJECT. Geldige waardes zijn datumtijden gelegen op of voor de huidige datum en tijd. | string | ja | ~~C~~​R​~~U~~​~~D~~ |
...
* `overige` - Overige |  | ja | C​R​U​D |
| objectTypeOverige | Beschrijft het type OBJECT als `objectType` de waarde &quot;overige&quot; heeft. | string | nee | C​R​U​D |
| objectTypeOverigeDefinitie | Verwijzing naar het schema van het type OBJECT als `objectType` de waarde &quot;overige&quot; heeft.
...
Indien je hier gebruikt van maakt, dan moet je een OBJECT url opgeven en is het gebruik van objectIdentificatie niet mogelijk. De opgegeven OBJECT url wordt gevalideerd tegen het schema van het opgegeven objecttype. |  | nee | C​R​U​D |
...
| Attribuut | Omschrijving | Type | Verplicht | CRUD* |
| --- | --- | --- | --- | --- |
| natuurlijkPersoon |  |  | nee | C​R​U​D |
| nietNatuurlijkPersoon |  |  | nee | C​R​U​D |
