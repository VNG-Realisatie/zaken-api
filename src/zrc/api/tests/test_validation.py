...
class ZaakValidationTests(JWTAuthMixin, APITestCase):
    ...
    def test_create_zaak_validate_incorrect_zaaktype_resource(self, *mocks):
        responses = {ZAAKTYPE: {"some": "incorrect property"}}
    ...
    @override_settings(LINK_FETCHER="vng_api_common.mocks.link_fetcher_200")
    def test_validate_communicatiekanaal_invalid_resource(
...
class DeelZaakValidationTests(JWTAuthMixin, APITestCase):
    scopes = [SCOPE_ZAKEN_BIJWERKEN, SCOPE_ZAKEN_CREATE]
    zaaktype = "https://example.com/foo/bar"
    ...
    def test_cannot_have_multiple_levels(self):
        ...
        deelzaak = ZaakFactory.create(
            hoofdzaak=hoofdzaak, zaaktype="https://example.com/foo/bar"
        )
...
class ZaakInformatieObjectValidationTests(
    ...
    def test_informatieobject_invalid_resource(self):
        responses = {INFORMATIEOBJECT: {"some": "incorrect property"}}
        ...
        with mock_client(responses):
            response = self.client.post(
                list_url, {"zaak": zaak_url, "informatieobject": INFORMATIEOBJECT}
            )
    ...
    def test_informatieobject_no_zaaktype_informatieobjecttype_relation(self, *mocks):
        ...
        with mock_client(RESPONSES):
            response = self.client.post(
                url, {"zaak": zaak_url, "informatieobject": INFORMATIEOBJECT}
            )
...
class ResultaatValidationTests(JWTAuthMixin, APITestCase):
    ...
    def test_resultaattype_invalid_resource(self, *mocks):
        ...
        responses = {RESULTAATTYPE: {"some": "incorrect property"}}

        with mock_client(responses):
            response = self.client.post(
                list_url, {"zaak": zaak_url, "resultaattype": RESULTAATTYPE}
            )
    ...
    def test_resultaattype_incorrect_zaaktype(self, *mocks):
        ...
        responses = {RESULTAATTYPE: {"url": RESULTAATTYPE, "zaaktype": ZAAKTYPE2}}
...
class ZaakEigenschapValidationTests(JWTAuthMixin, APITestCase):
    ...
    def test_eigenschap_invalid_resource(self):
        ...
        responses = {
            "http://ztc.com/eigenschappen/1234": {"some": "incorrect property"}
        }
